...
function buildPrompt(title, price, description) {
  return `# 目的
与えられた「商品情報」をもとに、ECサイトの一覧ページ用キャッチコピーと、詳細ページ用の説明文（ライティング）を作成する。

# 役割・トーン＆マナー
- 高級感と洗練された印象を与えるファッション・ライフスタイル系ECのコピーライターとして執筆する。
- 語尾・文体：「〜だ。」「〜である。」（だ・である調）で統一する。
- 事実に基づいた具体的な特徴（素材、構造、ディテール、サイズ感）を端的に伝える、洗練された文章に仕上げる。過度な飾りの形容詞は避ける。
- 推測で断定しない。原文に無い情報は書かない。公式説明が取得できていない場合は、商品名から確実に言える範囲に留める。英語原文は忠実に和訳して要点を整える。
...
- 文字数：20文字〜30文字程度（厳守）
- ブランド名や商品名、および最も特徴的なディテール（素材、柄、フォルム、カラーなど）をギュッと凝縮した魅力的でコンパクトな一文にする。
- 文字数の括弧書きは付けない。本文のみを出力する。

2. 【詳細ページ用説明文】（JSONキー: desc）
- 文字数：130文字〜170文字程度（厳守）
- 文末に「（〇〇文字）」と正確な文字数を必ず記載する（例: 「〜イタリア製の一品だ。（171文字）」）。
- ブランドや商品の素材、構造、ディテール、機能性、着用感（軽量さや履き心地など）を網羅し、客観的かつ魅力的に記述する。

# 出力形式
以下のJSONで出力する:
{"pr": "<キャッチコピー本文20〜30字>", "desc": "<説明文130〜170字>（〇〇文字）"}
